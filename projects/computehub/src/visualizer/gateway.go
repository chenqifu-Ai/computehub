...
// BridgeKernel 桥接 kernel，启用真实数据同步
func (vg *VisualizerGateway) BridgeKernel(k *kernel.ExtendedKernel) {
	vg.kernelMu.Lock()
	defer vg.kernelMu.Unlock()
	vg.bridgedKernel = k
	log.Printf("[Visualizer] Bridge kernel connected")
}

// SyncFromKernel 从 kernel 同步真实节点数据到 GPM
func (vg *VisualizerGateway) SyncFromKernel() {
	vg.kernelMu.Lock()
	kernel := vg.bridgedKernel
	vg.kernelMu.Unlock()
	...
	nodes := kernel.NodeMgr.ListNodes()
	if len(nodes) == 0 {
		return
	}

	log.Printf("[Visualizer] Syncing %d nodes from kernel", len(nodes))

	// 按区域分组
	regionMap := make(map[string]*RegionData)
	for _, nd := range nodes {
		region := nd.Register.Region
		if _, ok := regionMap[region]; !ok {
			regionMap[region] = &RegionData{Name: region}
		}
		regionData := regionMap[region]

		nodeData := NodeVisual{
			ID:         nd.Register.NodeID,
			Region:     region,
			GPUType:    nd.Register.GPUType,
			Status:     "online",
			CPUCores:   nd.Register.CPUCores,
			MemoryGB:   nd.Register.MemoryGB,
			Load:       0,
			ActiveTasks: 0,
		}

		if nd.Metrics != nil {
			nodeData.ActiveTasks = nd.Metrics.ActiveTasks
			nodeData.Load = nd.Metrics.CPUUtilization
		}

		regionData.Nodes = append(regionData.Nodes, nodeData)
		regionData.TotalNodes++
		regionData.OnlineNodes++
	}

	// 更新 GPM
	vg.gpm.mu.Lock()
	defer vg.gpm.mu.Unlock()

	for name, rd := range regionMap {
		vg.gpm.regions[name] = rd
	}
}
