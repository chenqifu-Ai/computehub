...
class ExpertHeartbeatSystem:
    ...
    def create_expert_scripts(self):
        ...
        for expert, config in self.experts_config.items():
            # 生成脚本内容
            expert_var = expert.replace('（', '_').replace('）', '').replace(' ', '_')
...
def {expert_var}_task():
    ...
    work_log["status"] = "已完成"
    work_log["output_file"] = f"/root/.openclaw/workspace/expert_work_logs/{expert_var}_report_{{datetime.now().strftime('%Y%m%d_%H%M%S')}}.json"
