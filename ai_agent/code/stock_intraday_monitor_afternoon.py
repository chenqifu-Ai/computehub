...
def is_afternoon_trading_hours():
    """检查是否在下午交易时间内"""
...
def generate_afternoon_status_report():
    ...
    # 添加下午盘特别提醒
    report += "\n💡 下午盘注意事项:\n"
    report += "• 关注尾盘资金流向和成交量变化\n"
    report += "• 注意主力资金是否护盘或出货\n"
    report += "• 如有持仓，考虑是否需要调整止盈止损位\n"
    
    return report

def main():
    print("🔍 启动股票监控系统 - 下午盘中任务...")
    print(f"⏰ 开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # 初始状态报告
    status_report = generate_afternoon_status_report()
    print("\n" + status_report)
    ...
    with open(f"{result_dir}/stock_intraday_afternoon_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", 'w', encoding='utf-8') as f:
    ...
    while True:
        try:
            ...
            # 检查是否还在下午交易时间
            if not is_afternoon_trading_hours():
                print("🕒 下午交易时间结束，暂停监控...")
                send_alert("下午交易时间结束，监控系统暂停", "INFO")
                break
            
            # 每5次检查输出一次状态报告（约2.5分钟）
            if check_count % 5 == 0:
                print(generate_afternoon_status_report())
