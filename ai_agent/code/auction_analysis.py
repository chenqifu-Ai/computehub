...
class AuctionAnalyzer:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if match:
                    ...
                    if len(values) > 40:  # 集合竞价相关数据通常在后面的位置
                        auction_data = {
                            'symbol': symbol,
                            'current_price': float(values[3]) if values[3] else 0,
                            'yesterday_close': float(values[4]) if values[4] else 0,
                            'opening_price': float(values[5]) if values[5] else 0,
                            'volume': int(values[13]) if values[13] else 0,
                            'turnover': float(values[37]) if values[37] else 0,
                            'bid_volume': int(values[17]) if values[17] else 0,  # 买一量
                            'ask_volume': int(values[19]) if values[19] else 0,  # 卖一量
                            'price_change_percent': ((float(values[3]) - float(values[4])) / float(values[4]) * 100) if values[3] and values[4] and float(values[4]) != 0 else 0
                        }
                        return auction_data
        except Exception as e:
    ...
    def analyze_auction(self, auction_data: Dict) -> Dict:
        ...
        # 计算量比（需要历史平均成交量，这里简化处理）
        if auction_data['volume'] > 0:
            analysis['volume_ratio'] = min(auction_data['volume'] / 1000000, 10)  # 简化计算
