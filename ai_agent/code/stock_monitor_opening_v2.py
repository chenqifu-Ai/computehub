...
class StockMonitorOpening:
    ...
    def get_stock_data_sina(self, symbol):
        ...
        try:
            # 确定市场前缀
            if symbol in ["399001", "399006", "000832"] or symbol.startswith(('00', '30')):
                market_prefix = "sz"
            else:
                market_prefix = "sh"
            ...
            if response.status_code == 200:
                ...
                if data_str and '=""' not in data_str:  # 检查是否有有效数据
                    # 解析新浪返回的数据
                    parts = data_str.split('"')[1].split(',')
                    if len(parts) > 31:  # 确保有足够的数据字段
                        stock_info = {
                            "symbol": symbol,
                            "name": parts[0],
