    ...
    def get_process_data(self):
        ...
        try:
            ...
            for line in lines:
                ...
                # 分析进程风险
                risk_level, alerts = self.analyze_process(process_name, cmd, user)
...
class DashboardHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/':
            monitor = ProcessMonitorDashboard()
...
def start_dashboard(port=8080):
    ...
    print(f"🚀 进程监控驾驶舱已启动: http://localhost:{port}")
    print(f"📊 监控频率: 每30秒自动刷新")
