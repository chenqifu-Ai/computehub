...
type DispatchEngine struct {
	...
	// MockMode 如果为 true，executeSubtask 不发起真实 API 调用，直接返回成功
	MockMode bool
...
func (e *DispatchEngine) executeSubtask(ctx context.Context, task DecomposedTask) SubtaskExecution {
	...
	// MockMode: 不发起真实 API 调用，直接返回成功
	if e.MockMode {
		return SubtaskExecution{
			SubtaskID:    task.ID,
			Success:      true,
			Result:       fmt.Sprintf("[mock] task %s executed on %s", task.ID, model),
			ModelUsed:    model,
