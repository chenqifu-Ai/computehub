...
type NodeRegister struct {
	...
	Platform      string    `json:"platform"`  // "linux" | "windows" | "darwin" | "android"
	...
	Version       string    `json:"version"` // 节点 Agent 版本号
}
...
type TaskSubmit struct {
	...
	Platform     string    `json:"platform"`    // "linux" | "windows" | "darwin" (auto-schedule filter)
	...
	SubmittedBy  string    `json:"submitted_by,omitempty"`  // who/what submitted this task (e.g. "gateway", "agent:xiaozhi", "cron", "system")
...
type NodeManagerState struct {
	...
	Suspect   bool // ARC-NET v1.1: 两阶段离线检测
}
...
func (nm *NodeManager) RegisterNode(reg *NodeRegister) error {
	...
	if existing, exists := nm.nodes[reg.NodeID]; exists {
		...
		// 🧮 Recount ActiveTasks from actual task state on re-registration
		// Prevents counter drift when node reconnects (e.g. windows-mobile reboot)
		activeCount := 0
		for _, ts := range existing.Tasks {
			if ts.Status == "pending" || ts.Status == "running" {
				activeCount++
			}
		}
		existing.Metrics.ActiveTasks = activeCount

		if existing.Tasks == nil {
			existing.Tasks = make(map[string]*TaskState)
		}
		logWithTimestamp("[NodeMgr] 🔄 Updated register info for node %s (region=%s, gpu=%s) active_task=%d",
			reg.NodeID, reg.Region, reg.GPUType, activeCount)
		return nil
	}
...
func (nm *NodeManager) Heartbeat(nodeID string, metrics *GPUMetrics) error {
	...
	state.Suspect = false // ARC-NET: 心跳到达，取消可疑状态
...
func (nm *NodeManager) SubmitTask(task *TaskSubmit) error {
	...
	// Compat: if NodeID is set but AssignedNode isn't, fallback
...
func (nm *NodeManager) CompleteTask(taskID, nodeID string, result *TaskResult) error {
	...
	// Guard against double-completion: if already terminal, skip decrement
	if ts.Status == "completed" || ts.Status == "failed" || ts.Status == "cancelled" || ts.Status == "preempted" {
		logWithTimestamp("[NodeMgr] ⚠️  Task %s already in terminal state (%s), skipping CompleteTask",
			taskID, ts.Status)
		return fmt.Errorf("task %s already in terminal state %s", taskID, ts.Status)
	}
	...
	if state.Metrics.ActiveTasks > 0 {
		...
		logWithTimestamp("[NodeMgr] ⚠️  ActiveTasks underflow prevented for node %s (task %s)", nodeID, taskID)
	}
...
type TaskDispatcher struct {
	...
	wsHub  interface{} // WSHub (type asserted by Gateway)
}

// SetWSHub 注入 WSHub 引用（由 Gateway 调用）
func (td *TaskDispatcher) SetWSHub(hub interface{}) {
	td.wsHub = hub
}
...
func (td *TaskDispatcher) dispatch() {
	// 遍历所有节点的 pending 任务（WS 推送暂未实现，Worker 通过 HTTP 轮询兜底）
	nm := td.kernel.NodeMgr
	nm.mu.RLock()
	defer nm.mu.RUnlock()
	...
	if totalPending > 0 {
		logWithTimestamp("[TaskDispatcher] Found %d pending tasks (HTTP poll fallback active)", totalPending)
	}
}
...
// ReclaimTasksForNode 导出版本（供 gateway 包调用）
func (nm *NodeManager) ReclaimTasksForNode(nodeID string) {
	nm.reclaimTasksForNode(nodeID)
}

// WithLock 在写锁保护下执行 fn（ARC-NET 两阶段健康检查用）
func (nm *NodeManager) WithLock(fn func()) {
	nm.mu.Lock()
	defer nm.mu.Unlock()
	fn()
}

// GetNodes 返回内部节点映射表（调用者必须在 WithLock 临界区内）
func (nm *NodeManager) GetNodes() map[string]*NodeManagerState {
	return nm.nodes
}
