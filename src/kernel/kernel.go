...
func (k *OpcKernel) handleNodeHeartbeat(cmd Command) Response {
	...
	case map[string]interface{}:
		...
		// Extract version from heartbeat
		hbVersion, _ := p["version"].(string)

		// Auto-register node if not found (recover from Gateway restart)
		if err := k.maybeAutoRegisterNode(nodeID, func() {
			k.NodeMgr.RegisterNode(&NodeRegister{
				NodeID: nodeID, NodeType: "gpu", Status: "online",
				Version: hbVersion, RegisteredAt: time.Now(),
			})
		}); err != nil {
			return Response{Success: false, Error: err, Duration: time.Since(start).String()}
		}
		// Update IP and version from heartbeat
		if state, err := k.NodeMgr.GetNodeState(nodeID); err == nil {
			if ip, ok := p["ip_address"].(string); ok && ip != "" && ip != "0.0.0.0" && state.Register.IPAddress != ip {
				state.Register.IPAddress = ip
				logWithTimestamp("[Kernel] 📍 Updated node %s IP → %s", nodeID, ip)
			}
			if hbVersion != "" && state.Register.Version != hbVersion {
				state.Register.Version = hbVersion
				logWithTimestamp("[Kernel] 📍 Updated node %s version → %s", nodeID, hbVersion)
			}
		}
