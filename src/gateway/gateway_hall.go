...
func (g *OpcGateway) handleHallPost(w http.ResponseWriter, r *http.Request) {
	...
	// ── Hall 消息自动流入 TriggerEngine ──
	if g.TriggerEngine != nil && req.To != "narrator" {
		msgType := "chat"
		if req.To == "all" || req.To == "" {
			msgType = "broadcast"
		}
		results := g.TriggerEngine.IngestHallEvent(req.Content, msgType)
		if len(results) > 0 {
			logWithTimestamp("[Trigger-Hall] 📢 %d 条规则匹配: from=%s content=%s", len(results), req.From, truncateString(req.Content, 50))
			for _, r := range results {
				logWithTimestamp("[Trigger-Hall]   → %s (rule=%s score=%d)", r.Message, r.RuleID, r.Score)
			}
		}
	}
...
// ══════════════════════════════════════════════════════════════════════
// HallRoundTable — 圆桌讨论发起器
// ══════════════════════════════════════════════════════════════════════

// RoundTableTopic 一个讨论话题
type RoundTableTopic struct {
	Topic    string   `json:"topic"`    // Hall 话题
	Prompts  []string `json:"prompts"`  // 主持人抛出的问题列表（循环使用）
	Interval int      `json:"interval"` // 抛问题间隔（秒）
	Turn     int      `json:"turn"`     // 当前轮次
}

// HallRoundTable 圆桌讨论管理器
type HallRoundTable struct {
	mu       sync.RWMutex
	enabled  bool
	stopCh   chan struct{}
	topics   []RoundTableTopic
	currIdx  int
	sender   string // 发消息者身份
	hostName string
...
// NewHallRoundTable 创建圆桌讨论管理器
func NewHallRoundTable() *HallRoundTable {
	return &HallRoundTable{
		stopCh:   make(chan struct{}),
		sender:   "控制中心",
		hostName: "主持人",
		topics: []RoundTableTopic{
			{Topic: "general", Prompts: []string{
				"🤖 各位，来聊聊今天有什么新发现或要解决的问题？",
				"💡 分享一下最近学到的经验？",
				"🔍 有没有遇到什么有意思的问题？",
				"📊 你的节点当前状态如何？CPU/内存/任务完成情况？",
				"🎯 有什么想和其他节点讨论的技术话题？",
			}, Interval: 300},
		},
		recorder: TopicRecorder{maxRounds: 50},
	}
}

// Start 启动圆桌（隔 interval 秒抛一个问题）
func (rt *HallRoundTable) Start() {
	rt.mu.Lock()
	if rt.enabled {
		rt.mu.Unlock()
		return
	...
	interval := rt.topics[rt.currIdx].Interval
	rt.mu.Unlock()
...
// IsRunning 是否运行中
func (rt *HallRoundTable) IsRunning() bool {
	rt.mu.RLock()
	defer rt.mu.RUnlock()
	return rt.enabled
}
...
// throwNext 抛出下一个问题
func (rt *HallRoundTable) throwNext() {
	rt.mu.Lock()
	t := rt.topics[rt.currIdx]
	if len(t.Prompts) == 0 {
		rt.mu.Unlock()
		return
	}
	prompt := t.Prompts[t.Turn%len(t.Prompts)]
	t.Turn++
	rt.topics[rt.currIdx] = t
	rt.mu.Unlock()

	content := fmt.Sprintf("🌐 【圆桌讨论 #%d】%s\n\n（大家各抒己见，附上节点名即可）", t.Turn, prompt)
	PostHallMessage(t.Topic, rt.sender, rt.hostName, "all", content)
	...
	rt.recorder.mu.Lock()
	rt.recorder.rounds = append(rt.recorder.rounds, rec)
	if len(rt.recorder.rounds) > rt.recorder.maxRounds {
		rt.recorder.rounds = rt.recorder.rounds[len(rt.recorder.rounds)-rt.recorder.maxRounds:]
	}
	rt.recorder.mu.Unlock()
}

// SetInterval 设置当前话题的间隔
func (rt *HallRoundTable) SetInterval(seconds int) {
	rt.mu.Lock()
	defer rt.mu.Unlock()
	if len(rt.topics) > 0 {
		rt.topics[rt.currIdx].Interval = seconds
	}
}

// GetHistory 获取讨论记录
func (rt *HallRoundTable) GetHistory() []RoundRecord {
	rt.recorder.mu.RLock()
	defer rt.recorder.mu.RUnlock()
	out := make([]RoundRecord, len(rt.recorder.rounds))
	copy(out, rt.recorder.rounds)
	return out
}

// AddReply 记录某个节点的回复（由 Hall 消息回调调用）
func (rt *HallRoundTable) AddReply(questionTopic, nodeID, content string) {
	rt.recorder.mu.Lock()
	defer rt.recorder.mu.Unlock()
	if len(rt.recorder.rounds) == 0 {
		return
	}
	last := &rt.recorder.rounds[len(rt.recorder.rounds)-1]
	last.Replies = append(last.Replies, struct {
		Node    string `json:"node"`
		Content string `json:"content"`
	}{Node: nodeID, Content: truncateString(content, 200)})
}

// ── Gateway 端点 ──

// handleRoundTableStart POST — 启动圆桌
func (g *OpcGateway) handleRoundTableStart(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, `{"error":"Only POST allowed"}`, http.StatusMethodNotAllowed)
		return
	}
	if g.RoundTable == nil {
		g.RoundTable = NewHallRoundTable()
	}
	g.RoundTable.Start()
	g.sendJSON(w, Response{Success: true, Data: map[string]interface{}{"running": true}})
}

// handleRoundTableStop POST — 停止圆桌
func (g *OpcGateway) handleRoundTableStop(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, `{"error":"Only POST allowed"}`, http.StatusMethodNotAllowed)
		return
	}
	...
	g.sendJSON(w, Response{Success: true, Data: map[string]interface{}{"running": false}})
}

// handleRoundTableStatus GET — 圆桌状态
func (g *OpcGateway) handleRoundTableStatus(w http.ResponseWriter, r *http.Request) {
	running := false
	if g.RoundTable != nil {
		running = g.RoundTable.IsRunning()
	}
	g.sendJSON(w, Response{
		Success: true,
		Data: map[string]interface{}{
			"running": running,
			"interval_sec": func() int {
				if g.RoundTable == nil { return 300 }
				return g.RoundTable.topics[0].Interval
			}(),
		},
	})
}

// handleRoundTableHistory GET — 讨论记录
func (g *OpcGateway) handleRoundTableHistory(w http.ResponseWriter, r *http.Request) {
	history := []RoundRecord{}
	if g.RoundTable != nil {
		history = g.RoundTable.GetHistory()
	}
	g.sendJSON(w, Response{
		Success: true,
		Data:    history,
...
// handleRoundTableInterval POST — 设置间隔
func (g *OpcGateway) handleRoundTableInterval(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, `{"error":"Only POST allowed"}`, http.StatusMethodNotAllowed)
		return
	}
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		g.sendJSON(w, Response{Success: false, Error: "invalid request"})
		return
	}
	if g.RoundTable != nil {
		g.RoundTable.SetInterval(req.Interval)
	}
	g.sendJSON(w, Response{Success: true, Data: map[string]interface{}{"interval": req.Interval}})
}

// handleRoundTableNow POST — 立即抛一个问题
func (g *OpcGateway) handleRoundTableNow(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, `{"error":"Only POST allowed"}`, http.StatusMethodNotAllowed)
		return
	}
	if g.RoundTable != nil {
		g.RoundTable.throwNext()
	}
	g.sendJSON(w, Response{Success: true})
}
