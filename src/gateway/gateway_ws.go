...
func (h *WSHub) FanOut(topic string, msg *WSMessage, exceptID string) int {
	...
	for _, snap := range snapshots {
		...
		go func(nid string, c *websocket.Conn, mu *sync.Mutex) {
			...
			if err == nil {
				// BUGFIX: 写成功后重置 deadline，避免残留过期 deadline
				// 影响 writePump 的 ping 写入（i/o timeout 风暴）
...
func (h *WSHub) writePump(client *WSClient, done chan struct{}) {
	...
	for {
		...
		case <-done:
			...
			return
		case <-ticker.C:
			// BUGFIX: 必须每次写前设置新的 deadline，因为 FanOut/SendTo
			// 设置的绝对时间 deadline 会残留，导致后续 ping 写入失败（i/o timeout）
			client.WriteMu.Lock()
