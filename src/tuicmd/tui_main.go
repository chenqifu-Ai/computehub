...
func screenNodes(state *AppState) {
	...
	// Table header (9 columns: NodeID | Region | GPU | Health | Tasks | IP | JoinTime | Lat | GPU)
		fmt.Printf(" %s%-20s │ %-10s │ %-10s │ %-8s │ %-6s │ %-12s │ %-8s │ %-7s │ %-10s │ %s%s\n",
		White+Bold, "Node ID", "Region", "GPU Type", "Health", "Tasks", "IP", "Join", "Lat(ms)", "GPU Ut/Temp", "Version", Reset)
	fmt.Printf(" %s%s%s\n", Dim, strings.Repeat("─", 128), Reset)
	...
	for _, n := range nodes {
		...
		fmt.Printf(" %s%-20s%s │ %-10s │ %-10s │ %s │ %s%4d%s  │ %-12s │ %-8s │ %s%7.0f%s │ %s / %-6s │ %s\n",
...
func screenTasks(state *AppState) {
	for { // loop until user goes back
		...
		for _, tasks := range taskMap {
			for _, t := range tasks {
				...
				case "completed":
					completedTasks++
				case "failed", "cancelled", "preempted":
					failedTasks++
				}
				switch {
				case t.Priority >= 9:
		...
		fmt.Printf("\n %s━━━ 任务概览 ━━━%s", Cyan+Bold, Reset)
		if activeTasks > 0 {
			fmt.Printf("  运行中: %s%3d%s", Yellow+Bold, activeTasks, Reset)
		} else {
			fmt.Printf("  运行中: %s%3d%s", Green+Bold, activeTasks, Reset)
		}
		if completedTasks > 0 {
			fmt.Printf("  已完成: %s%3d%s", Dim, completedTasks, Reset)
		}
		if failedTasks > 0 {
			fmt.Printf("  失败/取消: %s%3d%s", Red, failedTasks, Reset)
		}
		...
		type nodeTaskInfo struct {
			id, region, gpu, ip, version string
			active, max                  int
		}
		...
		for _, n := range nodes {
			...
			list = append(list, nodeTaskInfo{n.ID, n.Region, n.GPUType, ip, ver, n.ActiveTasks, n.MaxTasks})
		}
		sort.Slice(list, func(i, j int) bool { return list[i].active > list[j].active })

		// header
		fmt.Printf("  │ %-24s │ %-10s │ %-8s │ %-15s │ %-8s │ %-18s │\n", "Node", "Region", "GPU", "IP", "Version", "Load (active/max)")
		fmt.Printf("  │%s%s│\n", Dim, strings.Repeat("─", 96))
		...
		for _, li := range list {
			...
			fmt.Printf("  │ %-24s │ %-10s │ %-8s │ %-15s │ %s%-8s%s │ %s%-7s%s %s│\n",
				truncate(li.id, 24),
				li.region,
				li.gpu,
				li.ip,
				Green, li.version, Reset,
				pctColor, loadStr, Reset,
				renderBar(float64(li.active), float64(li.max), 10))
		}

	// ── 按时间过滤：只显示最近 10 分钟的任务 ──
	now := time.Now()
	var recentTasks []TaskListInfo
	for _, t := range allTasks {
		...
		// Parse "15:04:05" and compare against current time
		parsed, err := time.Parse("15:04:05", timeStr)
		if err != nil {
			continue // skip tasks with unparseable time
		}
		taskTime := time.Date(now.Year(), now.Month(), now.Day(),
			parsed.Hour(), parsed.Minute(), parsed.Second(), 0, now.Location())
		// Handle midnight rollover: if task time is more than 12h in the future, it's from yesterday
		if taskTime.After(now.Add(12*time.Hour)) {
			taskTime = taskTime.AddDate(0, 0, -1)
		}
		if now.Sub(taskTime) > 10*time.Minute {
			continue // skip tasks older than 10 minutes
		}
	...
	if len(recentTasks) > 0 {
		fmt.Printf("\n %s━━━ 任务明细（最近10分钟·时间降序）━━━%s\n", Cyan+Bold, Reset)
		fmt.Printf("  │ %-22s │ %-12s │ %-8s │ %-8s │ %-10s │ %-12s │ %-12s│ %-15s│\n",
			"Task ID", "Node", "Priority", "Status", "Time", "SubmittedBy", "Source", "IP")
		fmt.Printf("  │%s%s│\n", Dim, strings.Repeat("─", 115))

		sort.Slice(recentTasks, func(i, j int) bool {
			// Sort by time descending (newest first)
			getTime := func(t TaskListInfo) string {
				if t.SubmittedAt != "" { return t.SubmittedAt }
				return t.CreatedAt
			}
			return getTime(recentTasks[i]) > getTime(recentTasks[j])
		})
		...
		for _, t := range recentTasks {
			...
			fmt.Printf("  │ %-22s │ %-12s │ %s │ %s │ %-8s │ %-12s │ %-12s│ %-15s│\n",
				truncate(t.TaskID, 22),
				truncate(findNodeIDForTask(taskMap, t.TaskID), 12),
				visiblePad(prioStr, pc, Reset, 8),
				func() string {
			w := 8 - len(t.Status); if w < 0 { w = 0 }
			return statusColor(t.Status) + Reset + strings.Repeat(" ", w)
		}(),
				timeStr,
				func() string { s := t.SubmittedBy; if s == "" { s = "—" }; return truncate(s, 12) }(),
				truncate(t.Source, 12),
				truncate(ipStr, 15))
		}
		if len(recentTasks) == 0 {
			fmt.Printf("\n  %s无最近10分钟的任务%s\n", Dim, Reset)
		} else {
			fmt.Printf("  %s↑ 按时间降序排列（最近10分钟）| 数值 1-10, 10=Critical | 时间=提交时间 | 发起方=提交来源 | Source=类型 | IP=节点IP%s\n", Dim, Reset)
		}
	}
...
func screenShell(state *AppState) {
	// 已迁移到 tui_shell.go — 交互式远程终端
	clearScreen()
	fmt.Printf("%s╔══════════════════════════════════════╗%s\n", Cyan+Bold, Reset)
	fmt.Printf("%s║       💻 远程终端                      ║%s\n", Cyan+Bold, Reset)
	fmt.Printf("%s╚══════════════════════════════════════╝%s\n", Cyan+Bold, Reset)
	fmt.Printf("\n%s1. 交互式终端 (WS → Worker PTY)%s\n", Blue, Reset)
	fmt.Printf("%s2. OPC-Shell (Legacy HTTP)%s\n", Dim, Reset)
	fmt.Printf("%s0. 返回%s\n\n", Dim, Reset)

	input := strings.TrimSpace(readLine(" 选择 > "))
	switch input {
	case "1":
		screenInteractiveShell(state)
	case "2":
		screenShellLegacy(state)
	default:
		return
	}
}
