...
func registerMemorySyncTools(tr *agent.ToolRegistry, msc *MemorySyncClient) {
	...
	tr.Register(&agent.ToolEntry{
		Tool: agent.Tool{
			Name:        "memory_sync_knowledge",
			Description: "将验证过的知识同步到 Gateway 共享记忆层。topic=知识标题, content=知识内容",
			Parameters: []agent.Param{
				{Name: "topic", Type: "string", Required: true, Description: "知识标题"},
				{Name: "content", Type: "string", Required: true, Description: "知识内容"},
			},
		},
		Execute: func(ctx context.Context, args map[string]interface{}) (string, error) {
			...
			msc.SyncKnowledge(topic, content, []string{})
			return "✅ 知识已同步到共享记忆层", nil
		},
	})
}

// SyncPhase3Event 同步 Phase 3 事件到 Gateway
func (msc *MemorySyncClient) SyncPhase3Event(event agent.Phase3Event) {
	url := msc.gatewayURL + "/api/v1/galaxy/phase3/event"
	body, _ := json.Marshal(event)
	req, err := http.NewRequest("POST", url, bytes.NewReader(body))
	if err != nil {
		fmt.Printf(" [Phase3Sync:%s] ⚠️ 创建请求失败: %v\n", msc.nodeID, err)
		return
	}
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-Node-ID", msc.nodeID)
	resp, err := msc.httpClient.Do(req)
	if err != nil {
		fmt.Printf(" [Phase3Sync:%s] ⚠️ 同步事件失败: %v\n", msc.nodeID, err)
		return
	}
	defer resp.Body.Close()
}
